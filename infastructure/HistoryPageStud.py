...
#    Jan 05, 2021 04:46:01 PM +0200  platform: Windows NT
...
def vp_start_gui():
    ...
    global val, w, root
    root = tk.Tk()
    top = HistoryPageStud (root)
    HistoryPageStud_support.init(root, top)
    root.mainloop()

w = None
def create_HistoryPageStud(rt, *args, **kwargs):
    '''Starting point when module is imported by another module.
       Correct form of call: 'create_HistoryPageStud(root, *args, **kwargs)' .'''
    global w, w_win, root
    #rt = root
    root = rt
    w = tk.Toplevel (root)
    top = HistoryPageStud (w)
    HistoryPageStud_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_HistoryPageStud():
    global w
    w.destroy()
    w = None

class HistoryPageStud:
