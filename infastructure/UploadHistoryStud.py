...
import UploadHistoryStud_support
...
def vp_start_gui():
    ...
    global val, w, root
    root = tk.Tk()
    UploadHistoryStud_support.set_Tk_var()
    top = AssigUplHistory (root)
    UploadHistoryStud_support.init(root, top)
    root.mainloop()

w = None
def create_AssigUplHistory(rt, *args, **kwargs):
    '''Starting point when module is imported by another module.
       Correct form of call: 'create_AssigUplHistory(root, *args, **kwargs)' .'''
    global w, w_win, root
    #rt = root
    root = rt
    w = tk.Toplevel (root)
    UploadHistoryStud_support.set_Tk_var()
    top = AssigUplHistory (w)
    UploadHistoryStud_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_AssigUplHistory():
    global w
    w.destroy()
    w = None

class AssigUplHistory:
    def openmainmenu(self):
        root.destroy()
        Studentmainmenu.vp_start_gui()

    def download_file(self):
        mycol = connect_to_db_and_collection('EZSchooldb', 'HistoryAssignments')
    ...
    def upload_file(self):
        ...
        collection2 = connect_to_db_and_collection('EZSchooldb', 'HistoryAssignments')
        ...
        data = {'upload_time': upload_date,'file_name': filename, 'Assignment_file': encoded_string, 'Assignment_num': assign_num, 'Subject': 'History'}
        flag1 = 0
        flag2=0
    ...
    def __init__(self, top=None):
        ...
        self.values = list(range(1, connect_to_db_and_collection('EZSchooldb', 'HistoryAssignments').estimated_document_count() +1))
        self.newfile = None
