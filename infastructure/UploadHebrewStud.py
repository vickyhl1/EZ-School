...
import UploadHebrewStud_support
...
def vp_start_gui():
    ...
    global val, w, root
    root = tk.Tk()
    UploadHebrewStud_support.set_Tk_var()
    top = AssigUplHebrew (root)
    UploadHebrewStud_support.init(root, top)
    root.mainloop()

w = None
def create_AssigUplHebrew(rt, *args, **kwargs):
    '''Starting point when module is imported by another module.
       Correct form of call: 'create_AssigUplHebrew(root, *args, **kwargs)' .'''
    global w, w_win, root
    #rt = root
    root = rt
    w = tk.Toplevel (root)
    UploadHebrewStud_support.set_Tk_var()
    top = AssigUplHebrew (w)
    UploadHebrewStud_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_AssigUplHebrew():
    global w
    w.destroy()
    w = None

class AssigUplHebrew:
    def openmainmenu(self):
        root.destroy()
        Studentmainmenu.vp_start_gui()

    def download_file(self):
        mycol = connect_to_db_and_collection('EZSchooldb', 'HebrewAssignments')
    ...
    def upload_file(self):
        ...
        collection2 = connect_to_db_and_collection('EZSchooldb', 'HebrewAssignments')
        ...
        data = {'upload_time': upload_date,'file_name': filename, 'Assignment_file': encoded_string, 'Assignment_num': assign_num, 'Subject': 'Hebrew'}
        flag1 = 0
        flag2=0
    ...
    def __init__(self, top=None):
        ...
        self.values = list(range(1, connect_to_db_and_collection('EZSchooldb', 'HebrewAssignments').estimated_document_count() +1))
        self.newfile = None
